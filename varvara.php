...
// ----установка доступа к файлу с одного ip----
// if (true && $_SERVER ['REMOTE_ADDR'] != '178.49.5.184') {
...
/*---CONFIG---*/
$excludeDirs = "--exclude-dir=\"*wizards*\" --exclude-dir=\"*install*\" --exclude-dir=\"*cache*\"";
$maskfiles=array('.php','.htm','.html','.tpl','.txt','.inc','.js','.css'); //файлы, включающие эти слова идут в поиск
...
$matches = 0;
global $matches,$slow,$enc,$mask,$maskfiles,$maxfilesize,$db_host,$db_user,$db_pass,$excludeDirs;
...
<br>
	<input type="checkbox" name="cache"  checked>Моя борьба с cache/install/wizards</option>
...
if (strlen ( $text ) >= $minsearch) {
    ...
    if($where=="file")
    {
        if($step==1 && !$old_search)
        {
            ...
            if(!$excludeCache)
                $excludeDirs = "";
            $comm="grep -rlIF $masks ".escapeshellarg($text)." $excludeDirs $dir 2>/dev/null";
            if (PHP_OS === 'FreeBSD')
                $comm="grep -rlIF ".escapeshellarg($text)." $excludeDirs $dir 2>/dev/null";
            //echo $comm;
            $outp=wsoEx($comm);
