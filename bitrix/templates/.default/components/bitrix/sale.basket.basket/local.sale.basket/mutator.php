...
foreach ($this->basketItems as $row) {
	$product = CCatalogSku::GetProductInfo($row["PRODUCT_ID"]);
	if (is_array($product)) {
		$productsId[$row["PRODUCT_ID"]] = $product["ID"];
		$products[$row["PRODUCT_ID"]]["ID"] = $product["ID"];
	}
}

$elementGroups = CIBlockElement::GetElementGroups($productsId);
while ($sections = $elementGroups->GetNext()) {
	$key = array_search($sections["IBLOCK_ELEMENT_ID"], $productsId);
	$products[$key]["SECTION_NAME"] = $sections["NAME"];
	$products[$key]["SECTION_ID"] = $sections["ID"];

}

// Результирующий массив без дубликатов SECTION_NAME
$uniqueSectionNames = [];

// Проходим по исходному массиву и заменяем дубликаты на пустую строку
foreach ($products as $key => $item) {
    $sectionName = $item["SECTION_NAME"];
    
    if (!in_array($sectionName, $uniqueSectionNames)) {
        $uniqueSectionNames[] = $sectionName;
    } else {
        $products[$key]["SECTION_NAME"] = "";
    }
}
...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		...
		'SECTION_ID' => $products[$row['PRODUCT_ID']]["SECTION_ID"],
		'SECTION_NAME' => $products[$row['PRODUCT_ID']]["SECTION_NAME"],
