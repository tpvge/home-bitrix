...
<? if ($arResult['USER_VOTED'] == 'N') : ?>
    ...
    <div id="review_form" class="d-none">
        ...
        <div class="rating-area">
            <? for ($i = $arParams["STARS_COUNT"]; $i > 0; $i--) : ?>
                <input type="radio" id="star-<?= $i ?>" name="rating" <?= $arResult['USER_VOTED'] == 'Y' ? "disabled" : "" ?> checked value=<?= $i ?>>
...
<? else : ?>
    ...
    <div class="mt-2">
        <div class="font-weight-bold">Текст отзыва</div>
        <textarea name="review_text" id="review_text" rows="10" disabled class="w-50"><?= $arResult["COMMENT"] ?></textarea>
    </div>
<? endif; ?>
<script>
